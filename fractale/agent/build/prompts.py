...
requires = [
    ...
    "You MUST place executables on the PATH so they do not need to be discovered.",
...
'Do NOT add your narration unless it has a "#" prefix to indicate a comment.']  + requires
...
rebuild_task = f"""Your previous Dockerfile build has failed. Here is instruction for how to fix it.
...
def get_build_prompt(context):
    ...
    return Prompt(rebuild_prompt, context).render({"application": application, "environment": environment})
