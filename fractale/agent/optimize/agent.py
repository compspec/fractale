...
class OptimizationAgent(GeminiAgent):
    ...
    @timed
    def run(self, context, log, prompt=None):
        """
        ...
        prompt += prompts.supplement_optimize_prompt % (
        ...
        if "decision" not in result or "reason" not in result:
            prompt += "The JSON MUST have the fields 'decision', 'reason' with the manifest."
            return self.run(context, log, prompt)
