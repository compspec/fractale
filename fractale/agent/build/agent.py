...
class BuildAgent(Agent):
    ...
    def run(self, args, extra, error_message=None, dockerfile=None):
        """
        Run the agent.
        """
        ...
        # TODO need to add in dockerfile for rebuild, etc.
        ...
        return_code, output = self.build(
            dockerfile, image_name=args.container or self.generate_name(args.application)
        )
        if return_code == 0:
            print(
                Panel(
            ...
            return self.run(args, extra, error_message=output, dockerfile=dockerfile)
    ...
    def build(self, dockerfile, image_name):
        ...
        print(
            Panel(
                f"Attempting to build image: [bold cyan]{image_name}[/bold cyan]",
