...
generate_task = """I need to create a YAML manifest for a MiniCluster in an environment for '{{environment}}' for the exact container named '{{container}}'.
...
def get_generate_prompt(context, minicluster_explain):
    ...
    return Prompt(generate_prompt, context).render(
        {"environment": environment, "container": container, "minicluster": minicluster_explain}
    )
